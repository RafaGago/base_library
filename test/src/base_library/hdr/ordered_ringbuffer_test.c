...
#include <base_library/hdr/ordered_ringbuffer.h>
...
static void oringb_test_insert_head_fifo_duplicates (void **state)
{
  ...
  uword it   = oringb_insert_fifo_duplicates (&c->od, &next, nullptr);

  for (uword i = 0; i < oringb_capacity (&c->od) - 1; ++i) {
    next.key   = 0;
    next.value = i;
    it = oringb_insert_fifo_duplicates (&c->od, &next, nullptr);
  }
  uword i;
...
static void oringb_test_insert_head_lifo_duplicates (void **state)
{
  ...
  for (uword i = 0; i < oringb_capacity (&c->od) - 1; ++i) {
    next.key   = 0;
    next.value = i;
    it = oringb_insert_lifo_duplicates (&c->od, &next, nullptr);
  }
  for (uword i = 0; i < oringb_capacity (&c->od) - 1; ++i) {
    assert_true (oringb_at (&c->od, i)->key == 0);
    assert_true(
