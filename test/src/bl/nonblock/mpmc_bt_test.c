...
static void mpmc_bt_mpw_mpr (void **state)
{
  ...
  for (mpmc_bt_type i = 0; i < arr_elems (c->buff); ++i) {
     assert_true (mpmc_bt_produce (&c->q, &t, &i) == bl_ok);
     assert_true (mpmc_b_ticket_decode (t) == i);
     assert_true (mpmc_b_sig_decode (t) == 0);
  }
  assert_true(
    mpmc_bt_produce (&c->q, &t, &v) == bl_would_overflow
    );
  for (mpmc_bt_type i = 0; i < arr_elems (c->buff); ++i) {
    assert_true (mpmc_bt_consume(&c->q, &t, &v) == bl_ok);
    assert_true (v == i);
    assert_true (mpmc_b_ticket_decode (t) == i);
    assert_true (mpmc_b_sig_decode (t) == 0);
  }
  assert_true(
    mpmc_bt_consume (&c->q, &t, &v) == bl_empty
  );
}
/*---------------------------------------------------------------------------*/
...
static void mpmc_bt_spw_spr (void **state)
{
  ...
  mpmc_bt_type  v;
  for (mpmc_bt_type i = 0; i < arr_elems (c->buff); ++i) {
     assert_true (mpmc_bt_produce_sp (&c->q, &t, &i) == bl_ok);
     assert_true (mpmc_b_ticket_decode (t) == i);
     assert_true (mpmc_b_sig_decode (t) == 0);
  }
  assert_true(
    mpmc_bt_produce_sp (&c->q, &t, &v) == bl_would_overflow
    );
  for (mpmc_bt_type i = 0; i < arr_elems (c->buff); ++i) {
    assert_true (mpmc_bt_consume_sc (&c->q, &t, &v) == bl_ok);
    assert_true (v == i);
    assert_true (mpmc_b_ticket_decode (t) == i);
    assert_true (mpmc_b_sig_decode (t) == 0);
  }
  assert_true(
    mpmc_bt_consume_sc (&c->q, &t, &v) == bl_empty
  );
}
/*---------------------------------------------------------------------------*/
#ifdef MPMC_B_TEST_WRAP /*slow execution*/
...
static void mpmc_bt_producer_signals_set (void **state)
{
  ...
  assert_true (mpmc_bt_produce (&c->q, &t, &v) == bl_ok);
...
static void mpmc_bt_producer_fallback (void **state)
{
  ...
  err = mpmc_bt_produce_fallback (&c->q, &t, &v, (mpmc_b_sig) -1, 1);
  assert_true (err == bl_preconditions);
  assert_true (mpmc_b_sig_decode (t) == 1);

  err = mpmc_bt_produce_fallback (&c->q, &t, &v, (mpmc_b_sig) -1, 2);
  assert_true (err == bl_ok);
  assert_true (mpmc_b_sig_decode (t) == 1);
  assert_true (mpmc_b_ticket_decode (t) == 0);
...
static void mpmc_bt_consumer_fallback (void **state)
{
  ...
  err = mpmc_bt_consume_fallback (&c->q, &t, &v, (mpmc_b_sig) -1, 1);
  assert_true (err == bl_preconditions);
  assert_true (mpmc_b_sig_decode (t) == 1);

  err = mpmc_bt_consume_fallback (&c->q, &t, &v, (mpmc_b_sig) -1, 2);
  assert_true (err == bl_ok);
  assert_true (mpmc_b_sig_decode (t) == 1);
  assert_true (mpmc_b_ticket_decode (t) == 0);
...
static void mpmc_bt_producer_signal_change (void **state)
{
  ...
  err = mpmc_bt_produce_sig (&c->q, &t, &v, 2);
  assert_true (err == bl_ok);
  assert_true (mpmc_b_sig_decode (t) == 1);

  err = mpmc_bt_produce_sig (&c->q, &t, &v, 3);
  assert_true (err == bl_ok);
  assert_true (mpmc_b_sig_decode (t) == 2);
}
/*---------------------------------------------------------------------------*/
static void mpmc_bt_consumer_signal_change (void **state)
{
  ...
  err = mpmc_bt_consume_sig (&c->q, &t, &v, 2);
  assert_true (err == bl_ok);
  assert_true (mpmc_b_sig_decode (t) == 1);

  err = mpmc_bt_consume_sig (&c->q, &t, &v, 3);
  assert_true (err == bl_ok);
  assert_true (mpmc_b_sig_decode (t) == 2);
}
/*---------------------------------------------------------------------------*/
