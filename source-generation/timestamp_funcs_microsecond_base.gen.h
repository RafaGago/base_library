...
*/
/*---------------------------------------------------------------------------*/
/* AUTOGENERATE: include/bl/base/impl/generated/timestamp/timestamp{BITS}_funcs_microsecond_base.h */
/*---------------------------------------------------------------------------*/
#ifndef __BL_TIMESTAMP{BITS}_FNS_MICROSECOND_BASE_H__
#define __BL_TIMESTAMP{BITS}_FNS_MICROSECOND_BASE_H__

/*this file will be included from other files that have the missing defines*/
#include <bl/base/static_integer_math.h>
/*---------------------------------------------------------------------------*/
#define BL_TIMESTAMP{BITS}_HAS_MICROSECOND_BASE 1
/*---------------------------------------------------------------------------*/
#define bl_tstamp{BITS}_to_sec_priv(ts)\
  ((bl_toffset{BITS}) (ts / bl_usec_in_sec))

#define bl_tstamp{BITS}_to_msec_priv(ts)\
  ((bl_toffset{BITS}) (ts / bl_usec_in_msec))
...
{
  return (bl_toffset{BITS}) bl_div_ceil (ts, bl_usec_in_sec);
}

static inline bl_toffset{BITS} bl_tstamp{BITS}_to_msec_ceil (bl_tstamp{BITS} ts)
{
  return (bl_toffset{BITS}) bl_div_ceil (ts, bl_usec_in_msec);
}
