...
Don't edit.
*/
/* AUTOGENERATE: include/bl/base/impl/generated/timestamp/timestamp_funcs_microsecond_base.h */
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
#define bl_tstamp_to_sec_priv(ts)\
  ((bl_toffset) (ts / bl_usec_in_sec))

#define bl_tstamp_to_msec_priv(ts)\
  ((bl_toffset) (ts / bl_usec_in_msec))

#define bl_tstamp_to_usec_priv(ts)\
  ((bl_toffset) ts)

#define bl_tstamp_to_nsec_priv(ts)\
  (((bl_toffset) ts) * bl_nsec_in_usec)
/*---------------------------------------------------------------------------*/
#define bl_sec_to_tstamp_max()\
  bl_tstamp_to_sec_priv (bl_tstamp_max_safe_add_sub)

#define bl_msec_to_tstamp_max()\
  bl_tstamp_to_msec_priv (bl_tstamp_max_safe_add_sub)

#define bl_nsec_to_tstamp_max()\
  bl_tstamp_to_nsec_priv (bl_tstamp_max_safe_add_sub)

#define bl_usec_to_tstamp_max()\
  bl_tstamp_to_usec_priv (bl_tstamp_max_safe_add_sub)
/*---------------------------------------------------------------------------*/
static inline bl_toffset bl_tstamp_to_sec (bl_tstamp ts)
{
  return bl_tstamp_to_sec_priv (ts);
}

static inline bl_toffset bl_tstamp_to_msec (bl_tstamp ts)
{
  return bl_tstamp_to_msec_priv (ts);
}

static inline bl_toffset bl_tstamp_to_usec (bl_tstamp ts)
{
  return bl_tstamp_to_usec_priv (ts);
}

static inline bl_toffset bl_tstamp_to_nsec (bl_tstamp ts)
{
  return bl_tstamp_to_nsec_priv (ts);
}
/*---------------------------------------------------------------------------*/
